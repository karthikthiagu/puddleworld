...
class Experiment:
    ...
    def __init__(self, episodes, identifier, sarsa_lambda, goal):
        ...
        self.setLambda()
        self.identifier = '%s-%s-%s' % ( str(identifier), str(self.goal), str(sarsa_lambda) )  # The identifier is unique to this experiment, all data will be saved with this identifier
    ...
    def setLambda(self):
        RLGlue.RL_agent_message("set-lambda %s" % str(self.sarsa_lambda))
    ...
    def save(self):
        RLGlue.RL_agent_message("save_policy results/sarsa_lambda/%s.dat" % self.identifier)   # Save the value function
        returns = open('results/sarsa_lambda/%s_returns.npy' % self.identifier, 'w') # File to save the per-episode returns
        steps   = open('results/sarsa_lambda/%s_steps.npy'   % self.identifier, 'w')   # File to save the per-episode steps-to-the-goal
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 3:
        print 'Usage : python scripts/sarsa_lambda/experiment.py experiment_start experiment_end episodes lambda goal '
        exit()
    else:
        _, experiment_start, experiment_end, episodes, sarsa_lambda, goal = sys.argv
        experiment_start = int(experiment_start)
        experiment_end = int(experiment_end)
        episodes = int(episodes)
        sarsa_lambda = float(sarsa_lambda)

    for index in range(experiment_start, experiment_end + 1, 1):
        experiment = Experiment(episodes, '%d' % index, sarsa_lambda, goal)                  # Initialize the agent
