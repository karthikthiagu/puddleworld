...
class SarsaLAgent(Agent):
    ...
    def agent_init(self,taskSpecString):
        ...
        if TaskSpec.valid:
            ...
            self.etraces = [ self.numActions * [0.0] for i in range(self.numStates) ]
    ...
    def egreedy(self, state):
        if not self.exploringFrozen and self.randGenerator.random()<self.sarsal_epsilon:
    ...
    def agent_start(self,observation):
        ...
        # Initialize the eligibility traces to zero
        for state in range(self.numStates):
            for action in range(self.numActions):
                self.etraces[state][action] = 0
    ...
    def agent_step(self,reward, observation):
        ...
        sarsa_error_t = (reward + self.sarsal_gamma * Q_sprime_aprime - Q_sa)   # delta_t = the sarsa error
        self.etraces[lastState][lastAction] += 1                                # rev up the traces for this state

        if not self.policyFrozen:
            for state in range(self.numStates):
                for action in range(self.numActions):
                    self.value_function[state][action] += self.sarsal_stepsize * self.etraces[state][action] * sarsa_error_t
                    self.etraces[state][action] *= self.sarsal_lambda * self.sarsal_gamma
    ...
    def agent_end(self,reward):
        ...
        Q_sa = self.value_function[lastState][lastAction]                       # Q(S_t, A_t)
        sarsa_error_t = (reward - Q_sa)                                         # delta_t = the sarsa error
        self.etraces[lastState][lastAction] += 1                                # rev up the traces for this state
    ...
    def agent_message(self,inMessage):
        ...
        if inMessage.startswith("set-lambda"):
            _, sarsal_lambda = inMessage.split(' ')
            self.sarsal_lambda = float(sarsal_lambda)
        ...
        return "SarsaLAgent(Python) does not understand your message."



if __name__=="__main__":
    os.system('clear')
    AgentLoader.loadAgent(SarsaLAgent())
