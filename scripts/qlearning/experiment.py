...
class Experiment:
    ...
    def save(self):
        RLGlue.RL_agent_message("save_policy results/qlearning/%s.dat" % self.identifier)   # Save the value function
        returns = open('results/qlearning/%s_returns.npy' % self.identifier, 'w') # File to save the per-episode returns
        steps   = open('results/qlearning/%s_steps.npy'   % self.identifier, 'w')   # File to save the per-episode steps-to-the-goal
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 3:
        print 'Usage : python scripts/qlearning/experiment.py experiment_start experiment_end episodes goal'
        exit()
    else:
