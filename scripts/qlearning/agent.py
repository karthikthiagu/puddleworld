...
class QLearningAgent(Agent):
    ...
    def egreedy(self, state):
        if not self.exploringFrozen and self.randGenerator.random()<self.qlearning_epsilon:
    ...
    def agent_step(self,reward, observation):
        ...
        newState = observation.intArray[0]                # This is S_t+1
        lastState = self.lastObservation.intArray[0]      # This is S_t
        lastAction = self.lastAction.intArray[0]          # This is A_t
        
        Q_sa = self.value_function[lastState][lastAction] # This is Q(S_t, A_t)
        Q_sprime_aprime = max(self.value_function[newState][:]) # This is max( Q(S_t+1, a) )

        new_Q_sa = Q_sa + self.qlearning_stepsize * (reward + self.qlearning_gamma * Q_sprime_aprime - Q_sa)

        if not self.policyFrozen:
            self.value_function[lastState][lastAction]=new_Q_sa

        # Now take an action with this policy
        newIntAction=self.egreedy(newState) # This is A_t+1
    ...
    def agent_end(self,reward):
        ...
        new_Q_sa = Q_sa + self.qlearning_stepsize * (reward - Q_sa)
    ...
    def agent_message(self,inMessage):
        ...
        return "QLearningAgent(Python) does not understand your message."



if __name__=="__main__":
    os.system('clear')
    AgentLoader.loadAgent(QLearningAgent())
