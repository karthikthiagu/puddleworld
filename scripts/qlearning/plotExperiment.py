...
def compute(experiments, episodes, goal):
    ...
    for index in range(experiments):
        temp_returns = np.load( open('results/qlearning/%d-%s_returns.npy' % (index + 1, goal), 'r' ) )
        temp_steps   = np.load( open('results/qlearning/%d-%s_steps.npy' % (index + 1, goal)  , 'r' ) )
...
def conclude(xepisodes, returns, steps, goal):
    ...
    returns_plot = {'figure' : 1, 'color' : 'blue', 'xlabel' : 'Episodes', 'ylabel' : 'Average\n reward',\
                    'filename' : 'plots/qlearning/%s-returns.png' % goal}
    steps_plot   = {'figure' : 2, 'color' : 'blue', 'xlabel' : 'Episodes', 'ylabel' : 'Average\n episode\n length',\
                    'filename' : 'plots/qlearning/%s-steps.png' % goal}
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 4:
        print 'Usage : python scripts/qlearning/plotExperiment.py experiments episodes per_episode goal'
        exit()
    else:
